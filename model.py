...
class HybridTTS(nn.Module):
    def __init__(self, config):
        ...
        self.encoder = TokenEncoderWithFastText(fasttext_dim=config.fasttext_dim,
                                                 ...
                                                 dropout_rate=0.1,
                                                 use_projection=True)
        self.prosody_predictor = ProsodyPredictor(hidden_dim=config.acoustic_encoder_dim)
        self.upsample = DurationBasedUpsample()
        self.decoder = Tacotron2Decoder(hidden_dim=config.acoustic_encoder_dim, mel_dim=config.mel_dim)
        self.postnet = Postnet(mel_dim=config.mel_dim, postnet_dim=config.postnet_dim)
    ...
    def forward(self, token_ids, lengths, target_mel=None):
        ...
        duration, pitch, energy = self.prosody_predictor(enc_out)   # (B, T_enc) cho mỗi
        ...
        # Ví dụ:
        enc_upsampled = nn.utils.rnn.pad_sequence(enc_upsampled_list, batch_first=True)
        ...
        mel_output = self.decoder(enc_upsampled, target_mel, pitch=pitch, energy=energy)
