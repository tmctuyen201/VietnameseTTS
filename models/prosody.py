...
class ProsodyPredictor(nn.Module):
    ...
    def forward(self, enc_out):
        ...
        enc_out_flat = enc_out.view(-1, H)  # Làm phẳng thành (B * T, hidden_dim)
        
        # Truyền qua các lớp dự đoán
        duration = self.duration_pred(self.dropout(enc_out_flat)).view(B, T, 1)  # (B, T, 1)
        pitch = self.pitch_pred(self.dropout(enc_out_flat)).view(B, T, 1)  # (B, T, 1)
        energy = self.energy_pred(self.dropout(enc_out_flat)).view(B, T, 1)  # (B, T, 1)
