...
class TokenEncoderWithFastText(nn.Module):
    ...
    def __init__(self, fasttext_dim, lstm_dim, dropout_rate=0.1, use_projection=True):
        ...
        self.conv1 = nn.Conv1d(in_channels=lstm_dim,
                               out_channels=lstm_dim, kernel_size=3, padding=1)
        self.conv2 = nn.Conv1d(in_channels=lstm_dim,
                               out_channels=lstm_dim, kernel_size=3, padding=1)
        self.conv3 = nn.Conv1d(in_channels=lstm_dim,
                               out_channels=lstm_dim, kernel_size=3, padding=1)
    ...
    def forward(self, fasttext_embeddings, lengths):
        ...
        contextualized_features, _ = nn.utils.rnn.pad_packed_sequence(
